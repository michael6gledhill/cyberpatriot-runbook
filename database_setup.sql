...
-- 2. Create users table first (without team_id FK - will be added after teams table)
CREATE TABLE IF NOT EXISTS users (
    id INT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(255) NOT NULL,
    email VARCHAR(255) UNIQUE NOT NULL,
    password_hash VARCHAR(255) NOT NULL,
    role ENUM('member','captain','coach','mentor','admin','pending') NOT NULL DEFAULT 'pending',
    team_id INT,
...
CREATE TABLE IF NOT EXISTS teams (
    id INT AUTO_INCREMENT PRIMARY KEY,
    name VARCHAR(255) NOT NULL,
    team_id CHAR(7) NOT NULL UNIQUE,
...
-- Add the team_id foreign key constraint to users table now that teams exists
ALTER TABLE users ADD CONSTRAINT fk_users_team_id FOREIGN KEY (team_id) REFERENCES teams(id) ON DELETE SET NULL ON UPDATE CASCADE;

-- 5. Team join requests table (for competitors/coaches/mentors requesting to join teams)
CREATE TABLE IF NOT EXISTS team_join_requests (
    ...
    FOREIGN KEY (team_creator_user_id) REFERENCES users(id) ON DELETE CASCADE ON UPDATE CASCADE
);

-- 6. Team members table (for approval workflow)
CREATE TABLE IF NOT EXISTS team_members (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    team_id INT NOT NULL,
    status ENUM('approved','pending','rejected') NOT NULL DEFAULT 'pending',
    assigned_role ENUM('member','captain','coach','mentor') NOT NULL DEFAULT 'member',
...
-- 15. Create application MySQL user
